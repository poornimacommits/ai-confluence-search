...
load_dotenv()
API_KEY = os.getenv("AZURE_OPENAI_API_KEY").strip()  # Ensure your .env has
...
def extract_filters(query: str):
    ...
    - for keywords like "unused pages or stale pages", treat them as which are not modified in the last 2 years as a 
        special case and convert that to a lastmodified_timestamp filter with appropriate date and dont add them in the semantic_query field.
